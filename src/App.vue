<template>
  ...
  :style="xs?{height: '100%',width: '100%',top: '0',left:'0'}:(sm?{height: '98%',width: '98%',top: '1%',left:' 1%'}:{height: '96.6%',width: '99%',top: '1.7%',left:' 0.5%'})">
    ...
    <div v-show="!isloading && !isClearScreen" :style="xs||sm?{'overflow-y': 'auto','overflow-x': 'hidden'}:{}">
        <v-row>
            <v-col cols="12" md="4" lg="3" class="DazzlingDust-left" align="center">
              <div :style="xs||sm?{'font-size':'2.3rem'}:{'display':'none'}" class="DazzlingDust-left-welcome">{{ configdata.welcometitle }}</div>  
              <v-avatar class="DazzlingDust-left-avatar" :size="xs||sm?120:140" :style="xs||sm?{'margin-top': '0'}:{'margin-top': '2rem'}" @mouseenter="musicplayershow(1)" @mouseleave="musicplayershow(0)">
                  <v-img :class="{'DazzlingDust-spin':isPlaying}"
                  alt="DazzlingDust"
                ...
                <v-card class="ma-5 pa-2 DazzlingDust-left-card" variant="tonal" :max-width="xs?270:300" style="text-align: center; min-height: 150px;">
                ...
                <v-container class="DazzlingDust-left-socialIconsContainer">
                    <v-row align="center" justify="center">
                    <v-col class="pa-1" cols="auto" v-for="item in socialPlatformIcons">
                        <v-btn :size="xs?25:33" variant="tonal" color="var(--DazzlingDust-vcard-color)"
                        class="ma-1 DazzlingDust-social-bticon"
                        icon
                    ...
                    <v-col class="ma-1" cols="auto">
                        ...
                        <template v-slot:activator="{ props: activatorProps }">
                            <v-fab style="width: 2.5rem;height: 2.5rem;" color="var(--DazzlingDust-vcard-color)"
                            variant="tonal"
                            v-bind="activatorProps"
                            icon="mdi-cog"
                            ></v-fab>
                        </template>
                        <v-btn variant="tonal" class="setbtn" key="1" icon="mdi-key-chain" @click="dialog1 = true" size="31" color="var(--DazzlingDust-vcard-color)"></v-btn>
                        <v-btn variant="tonal" class="setbtn" key="2" icon="mdi-information" @click="dialog2 = true" size="31" color="var(--DazzlingDust-vcard-color)"></v-btn>
                        <v-btn variant="tonal" class="setbtn" key="3" icon="$error" size="31" color="var(--DazzlingDust-vcard-color)"></v-btn>
                        </v-speed-dial>
                    </v-col>
                    </v-row>
                </v-container>
    ...
    <v-dialog
      ...
      <v-card elevation="3" style="backdrop-filter: blur(10px);">
        <v-tabs
          ...
          align-tabs="center"
          height="60"
          slider-color=var(--DazzlingDust-vcard-color)
        >
      ...
      <v-card class="ma-3 pa-2" hover
        ...
        >
          <template v-slot:title >
            <span class="DazzlingDust-card-title">关于</span>
          </template>
          <div style="display: flex;flex-direction: column;align-items: center;">
            <v-card class="ma-3 pa-2" hover
              ...
              <template v-slot:subtitle>
                <span class="DazzlingDust-card-subtitle">本页基于以下技术及服务搭建</span>
              </template>
              <div>
