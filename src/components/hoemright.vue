<template>
      <div>
        <div>
          <div :style="xs||sm?{'display':'none'}:{'font-size':'4rem'}" class="DazzlingDust-left-welcome">{{ configdata.welcometitle }}</div>
        </div>
        <div>
          <v-row align="center">
            <v-col cols="12" md="8">
              ...
              <v-card class="ma-3" hover
                >
                  <template v-slot:title >
                    <span class="DazzlingDust-card-title clock-font">{{formattedTime}}</span>
                  </template>
          ...
          <v-chip class="mt-3 ml-3" prepend-icon="mdi-webhook"  size="large" style="color: var(--DazzlingDust-vcard-color);">
            部署项目
          </v-chip>
          <v-container>
            <v-row>
