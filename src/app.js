...
export default {
  ...
  data() {
    return {
      ...
      isExpanded: false,
      stackicons:[
        {icon:"simple-icons:vuedotjs",color:"green", model: false,tip: 'vue'},
        {icon:"simple-icons:javascript",color:"#CAD300", model: false,tip: 'javascript'},
        {icon:"simple-icons:css3",color:"blue", model: false,tip: 'css'},
        {icon:"simple-icons:html5",color:"red", model: false,tip: 'html'},
        {icon:"simple-icons:vuetify",color:"#1697F6", model: false,tip: 'vuetify'},
      ],
      projectcards:null,
      tab: null,
      tabs: [
  ...
  methods: {
    ...
    setMainProperty(imageurl){
      ...
      let DazzlingDustdata = this.getCookie("DazzlingDustdata");
      if(DazzlingDustdata){
        root.style.setProperty('--DazzlingDust-welcomtitle-color', `${DazzlingDustdata.color.welcometitlecolor}`);
        root.style.setProperty('--DazzlingDust-vcard-color', `${DazzlingDustdata.color.themecolor}`);
        root.style.setProperty('--DazzlingDust-brightness', `${DazzlingDustdata.brightness}%`);
        root.style.setProperty('--DazzlingDust-blur', `${DazzlingDustdata.blur}px`); 
      }else{
        root.style.setProperty('--DazzlingDust-welcomtitle-color', `${this.configdata.color.welcometitlecolor}`);
        root.style.setProperty('--DazzlingDust-vcard-color', `${this.configdata.color.themecolor}`);  
        root.style.setProperty('--DazzlingDust-brightness', `${this.configdata.brightness}%`);  
        root.style.setProperty('--DazzlingDust-blur', `${this.configdata.blur}px`);
      }
  
      let DazzlingDustdatabackground = this.getCookie("DazzlingDustdatabackground");
      const { xs } = useDisplay();
      if(DazzlingDustdatabackground){
        if(xs.value){
          if(DazzlingDustdatabackground.mobile.type == "pic"){
            root.style.setProperty('--DazzlingDust-background-image-url', `url('${DazzlingDustdatabackground.mobile.datainfo.url}')`);
            imageurl = DazzlingDustdatabackground.mobile.datainfo.url;
            return imageurl;
          }else{
            this.videosrc = DazzlingDustdatabackground.mobile.datainfo.url;
          }
        }else{
          if(DazzlingDustdatabackground.pc.type == "pic"){
            root.style.setProperty('--DazzlingDust-background-image-url', `url('${DazzlingDustdatabackground.pc.datainfo.url}')`);
            imageurl = DazzlingDustdatabackground.pc.datainfo.url;
            return imageurl;
          }else{
            this.videosrc = DazzlingDustdatabackground.pc.datainfo.url;
          }
        }
        ...
        if(xs.value){
          if(this.configdata.background.mobile.type == "pic"){
            root.style.setProperty('--DazzlingDust-background-image-url', `url('${this.configdata.background.mobile.datainfo.url}')`);
          ...
          if(this.configdata.background.pc.type == "pic"){
            root.style.setProperty('--DazzlingDust-background-image-url', `url('${this.configdata.background.pc.datainfo.url}')`);
